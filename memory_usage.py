...
def see_memory_usage_(message, force=True):
	logger = ''
	logger += message
	nvmlInit()
...
def see_memory_usage_MB(message, force=True):
	logger = ''
	logger += message
	nvmlInit()
	...
	handle = nvmlDeviceGetHandleByIndex(0)
	info = nvmlDeviceGetMemoryInfo(handle)
	logger += "\n Nvidia-smi: " + str((info.used) / 1024 / 1024 ) + " MB"
	# logger += message

	# logger += str(torch.cuda.memory_snapshot())
	# logger += str(torch.cuda.memory_summary(device=0, abbreviated=False))

	logger += '\n    Memory Allocated: '+str(torch.cuda.memory_allocated() / (1024 * 1024 )) +'  MegaBytes\n'
	logger +=   'Max Memory Allocated: ' + str(
		torch.cuda.max_memory_allocated() / (1024 * 1024 )) + '  MegaBytes\n'
